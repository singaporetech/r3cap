...
// import { CaptureSessionManager } from "../_deprecated/capture-session/CaptureSessionManager";
...
export class SocketHandler {
  ...
  {
    this.serverIP =  window.location.hostname + ":8000"; // if acccessible through local/private/lan networks, OR fctl servers
    ...
    // this.serverIP =  window.location.hostname; // if needs to be accessible in any other static public IP

    this.serverURL = "wss://" + this.serverIP + "/start_websocket";
    this.webSocket = null;
  ...
  ParseData(codeToClient: number, jsonData: any) {
    switch (codeToClient) {
      ...
      case SocketHandler.CodeToClient.EditRoom_ServerSend_SessionUpdate:
        SessionManager.instance.HandleRoomUpdate(jsonData);
        break;
      case SocketHandler.CodeToClient.New_Reconstruction:
        console.log("Received New_Reconstruction request!")
        SessionManager.instance.serverObjectManager.FindAndProcessLiveModelMasters(
          jsonData
        );
        ModelBrowserToolManager.instance.FetchModelPreviews()
      ...
      case SocketHandler.CodeToClient.Room_Previews_Update:
        SessionManager.instance.FetchRoomPreviews()
        break;
